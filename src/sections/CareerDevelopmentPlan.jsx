            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:gap-6 mb-6">
                <div>
                  ...
                  <p className="mt-2 text-sm dark:text-cyan-200 text-indigo-600 transition-colors duration-300">
                    {phase.timeline}
                  </p>
                </div>
                <div className="flex items-center justify-center h-12 w-12 rounded-full dark:bg-cyan-400/20 bg-indigo-600/20 flex-shrink-0">
                  <span className="text-lg font-bold dark:text-cyan-300 text-indigo-700">
                    {index + 1}
                  </span>
                </div>
              </div>
              <ul className="space-y-3">
                {phase.objectives.map((objective) => (
                  ...
                  >
                    <span className="text-indigo-600 dark:text-cyan-400 mt-1 font-bold">
                      ✓
                    </span>
                    <span>{objective}</span>
                  </li>
        ...
        <motion.div {...revealUp(0.4)} className="mt-12">
          <div className="glass-card rounded-[1.75rem] p-8 grid sm:grid-cols-3 gap-6">
            <div className="text-center sm:text-left">
              <p className="text-sm uppercase tracking-widest dark:text-cyan-200 text-indigo-600 transition-colors duration-300 font-semibold">
                Short Term (6 mo)
              </p>
              <p className="mt-3 text-lg font-semibold dark:text-white text-indigo-900 transition-colors duration-300">
          ...
          <div className="glass-card rounded-[1.75rem] p-8">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {careerGoals.targetRoles.map((role) => (
                <span
                  key={role}
                  className="rounded-full border transition-all duration-300 px-4 py-2 text-sm font-medium dark:border-cyan-200/30 dark:bg-cyan-400/10 dark:text-cyan-200 border-indigo-300 bg-indigo-100 text-indigo-700"
                >
                  {role}
                </span>
              ))}
